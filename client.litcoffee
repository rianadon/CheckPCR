...
Because this is run as a chrome extension, this page can be accessed. Otherwise, the browser would throw an error for security reasons (you don't want a random website being able to access confidential data from a website you have logged into).
    ...
    fetch = ->
      if Date.now()-lastUpdate < 60000 # Less than a minute ago
        return
      lastUpdate = Date.now()
    ...
    display = (doScroll=true) ->
      ...
      for s in split
        ...
        if s.assignment.end is "Forever"
          ...
          if Math.floor(s.start/1000/3600/24)-(if assignment.baseType is "test" then JSON.parse(localStorage["earlyTest"]) else 0) <= today <= Math.floor(s.end/1000/3600/24)
            e.classList.add "listDisp"
      ...
      if weekHeights[todayWkId]?
        ...
        if doScroll and document.body.getAttribute("data-view") is "0" #in calendar view
    ...
    for e in document.getElementsByClassName "settingsControl"
      ...
      e.addEventListener "change", (evt) ->
        ...
        if evt.target.name is "refreshRate"
          intervalRefresh()
        if evt.target.name is "earlyTest"
          display()
            ...
            if name is "for"
              ...
              for cls in data.classes
                ...
                if i is val.length-1
                  ...
                  document.getElementById(id).classList.toggle "active", cls.toLowerCase().indexOf(val.substr i+1) isnt -1
              return
