...
Now the functions that actually display something are defined. First up is one to add an activity line to the activity panel.
    ...
    addActivity = (type, assignment, newActivity, className=window.data.classes[assignment.class]) ->
      ...
      if assignment.id in done.array
        te.classList.add "done"
      insertTo.insertBefore te, insertTo.querySelector(".activity")
...
This function will convert the array of assignments generated by *parse* into readable HTML.
    ...
    display = (doScroll=true) ->
      ...
      timeafter = {
        ...
        us: 15*3600*1000
      }[localStorage["hideassignments"]] or 24*3600*1000
      ...
      for assignment, num in window.data.assignments
        ...
        if lastData?
          found = false
          for oldAssignment, num in lastData.assignments
      ...
      if lastData?
        ...
        for assignment in lastData.assignments
      ...
      for s in split
        ...
        separated = separate if assignment.class? then window.data.classes[assignment.class] else "Task"
        ...
        if (reference? and reference.done) or assignment.id in done.array
          e.classList.add "done"
        e.setAttribute "data-class", if s.custom and separateTaskClass then "Task" else window.data.classes[assignment.class]
        ...
        for a in activity.slice activity.length-32, activity.length
          if a[0] is "edit" and a[1].id is assignment.id
            ...
            te.addEventListener "click", ->
              ...
              resize() if document.body.getAttribute("data-view") is "1"
            mods.appendChild te
        e.appendChild mods

        if localStorageRead("assignmentSpan") == "multiple" and start < s.start
          e.classList.add "fromWeekend"
        if localStorageRead("assignmentSpan") == "multiple" and end > s.end
        ...
        if s.assignment.end is "Forever"
          ...
          if (st-(if assignment.baseType is "test" and assignment.start is st then JSON.parse(localStorage["earlyTest"]) else 0))*1000*3600*24+tzoff <= midDate.getTime() <= s.end.getTime()+(if listDateOffset is 0 then timeafter else 24*3600*1000)
            e.classList.add "listDisp"

        # Calculate how many assignments are placed before the current one
        if not s.custom or not JSON.parse localStorage["sepTasks"]
          pos = 0
          loop
            found = true
            d = new Date s.start
        ...
        if not s.custom or not JSON.parse localStorage["sepTasks"] then e.addEventListener "click", (evt) ->
          el = evt.target
          until el.classList.contains "assignment"
            el = el.parentNode
        ...
        if (assignment.baseType is "test" or (localStorageRead("projectsInTestPane") and assignment.baseType is "longterm")) and assignment.end >= today
          te = element "div", ["upcomingTest", "assignmentItem", assignment.baseType], "<i class='material-icons'>#{if assignment.baseType is "longterm" then "assignment" else "assessment"}</i><span class='title'>#{assignment.title}</span><small>#{separated[2]}</small><div class='range'>#{dateString(end, true)}</div>", "test"+assignment.id
          te.setAttribute "data-class", window.data.classes[assignment.class]
          id = assignment.id
          do(id) ->
            te.addEventListener "click", ->
        ...
        if already? # Assignment already exists
          ...
          already.setAttribute "data-class", if s.custom and separateTaskClass then "Task" else window.data.classes[assignment.class]
          ...
          already.classList.remove(cl) for cl in getES(already)
          already.classList.add(cl) for cl in getES(e)
          for cl in ["fromWeekend", "overWeekend"]
            already.classList.remove(cl)
            if e.classList.contains(cl) then already.classList.add(cl)
        else
          if s.custom and JSON.parse localStorage["sepTasks"]
            if assignment.start is st and assignment.end >= today
              e.classList.remove "assignment"
              e.classList.add "taskPaneItem"
              e.style.order = assignment.end
              if (link=e.querySelector(".linked"))?
      ...
      document.body.classList.toggle "noList", document.querySelectorAll(".assignment.listDisp:not(.done)").length is 0
...
Then, the tabs are made interactive.
    ...
    for tab in document.querySelectorAll "#navTabs>li"
      tab.addEventListener "click", (evt) ->
        if ga? then ga 'send', 'event', 'navigation', evt.target.textContent,
          page: '/new.html'
...
Additionally, the active class needs to be added when inputs are selected (for the login box).
    ...
    for input in document.querySelectorAll "input[type=text]:not(#newText):not([readonly]), input[type=password], input[type=email], input[type=url], input[type=tel], input[type=number]:not(.control), input[type=search]"
...
The event listener for the previous date button is mostly the same.
    ...
    document.getElementById("listbefore").addEventListener "click", ->
      pd = document.getElementById "listprevdate"
      td = document.getElementById "listnowdate"
      nd = document.getElementById "listnextdate"
      listDateOffset -= 1
...
Whenever a date is double clicked, long tapped, or force touched, list view for that day is displayed.
    ...
    switchToList = (evt) ->
      if evt.target.classList.contains("month") or evt.target.classList.contains("date")
        today = Math.floor (Date.now()-tzoff)/1000/3600/24
    ...
    do ->
      ...
      document.body.addEventListener "touchstart", (evt) ->
        taptimer = setTimeout (-> switchToList(evt)), 1000
...
The code below is what the settings control.
    ...
    document.body.classList.add "sepTaskClass" if JSON.parse localStorage["sepTaskClass"]
    ...
    if localStorageRead("data")? and not localStorage["classColors"]?
      a = {}
      for c in localStorageRead("data").classes
        a[c] = "#616161"
    ...
    for e in document.getElementsByClassName "settingsControl"
      ...
      e.addEventListener "change", (evt) ->
        ...
        switch evt.target.name
          ...
          when "assignmentSpan" then display()
          when "projectsInTestPane" then display()
          when "hideassignments" then display()
          when "holidayThemes" then document.body.classList.toggle "holidayThemes", evt.target.checked
          when "sepTaskClass" then document.body.classList.toggle "sepTaskClass", evt.target.checked; display()
          when "sepTasks" then document.getElementById("sepTasksRefresh").style.display = "block"
...
The same goes for textareas.
    ...
    for e in document.getElementsByTagName("textarea")
      if e.name isnt "athenaDataRaw" and localStorage[e.name]?
    ...
    class ParseArray
      ...
      update: (updatedData) ->
        ...
        while n < @array.length
          ...
          # then it was removed
          if updated.indexOf(item) is -1
            @onremove item
            @array.splice n, 1
          else
            n++
          updated.splice 0,1 # Remove it since we've already handled it
        # Now everything left in updated wasn't present in the old version
        # has been added
        for newitem in updated
          @onadd newitem
          @array.push newitem
...
The completed and modified assignments are loaded.
    ...
    done.onadd = (id) -> # Called when another device has marked an assignment as done
        for elem in document.querySelectorAll ".assignment[id^=\"#{id}\"], #test#{id}, #activity#{id}, #ia#{id}"
          elem.classList.add "done"
        document.body.classList.remove "noList" if document.querySelectorAll(".assignment.listDisp:not(.done)").length != 0
        resize() if document.body.getAttribute("data-view") is "1"
    done.onremove = (id) -> # Called when another device has marked an assignment as not done
        for elem in document.querySelectorAll ".assignment[id^=\"#{id}\"], #test#{id}, #activity#{id}, #ia#{id}"
    ...
    schedules = do -> if (s=localStorage["schedules"]) then JSON.parse(s) else null
...
The page to be sent to Google Analytics is set to the correct one.
    ...
    gp =
      page: '/new.html'
      title: if location.protocol is "chrome-extension:" then "Version #{version}" else "Online"
...
The show completed tasks checkbox is set correctly and is assigned an event listener.
    ...
    if localStorage["showDoneTasks"] and JSON.parse(localStorage["showDoneTasks"])
      document.getElementById("showDoneTasks").checked = true
      document.getElementById("infoTasksInner").classList.add "showDoneTasks"
    document.getElementById("showDoneTasks").addEventListener "change", ->
      localStorage["showDoneTasks"] = JSON.stringify this.checked
      document.getElementById("infoTasksInner").classList.toggle "showDoneTasks", this.checked
...
For updating, a request will be send to Github to get the current commit id and check that against what's stored
          ...
          if version isnt c
            ...
            send (if location.protocol is "chrome-extension:" then "https://api.github.com/repos/19RyanA/CheckPCR/git/refs/heads/master" else "/api/commit"), "json"
              .then (resp) ->
                ...
                send (if location.protocol is "chrome-extension:" then resp.response.object.url else "/api/commit/#{sha}"), "json"
                  .then (resp) ->
    ...
    if location.protocol is "chrome-extension:" then checkCommit()
