...
This function will convert the array of assignments generated by *parse* into readable HTML.
    ...
    display = ->
      ...
      for s in split
        ...
        e = element "div", ["assignment", assignment.baseType, "anim"], "<#{smallTag}#{if link? then " href='#{link}' class='linked' target='_blank'" else ""}><span class='extra'>#{separated[1]}</span>#{separated[2]}</#{smallTag}><span class='title'>#{assignment.title}</span><input type='hidden' class='due' value='#{assignment.end}' />", assignment.id+weekId
        if assignment.id in done
        ...
        do (id) ->
          edit.addEventListener "mouseup", (evt) ->
            if evt.which == 1
              ...
              done.style.display = if remove then "block" else "none"
            return
        ripple edit
        ...
        )(assignment.body, edits.querySelector(".additions"), edits.querySelector(".deletions"), edits, assignment.id)
        e.appendChild body
      ...
      if weekHeights[todayWkId]?
        ...
        if document.body.getAttribute("data-view") is "0" #in calendar view
