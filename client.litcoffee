    ...
    display = (doScroll=true) ->
      ...
      for s in split
        ...
        if s.assignment.end is "Forever"
          ...
          if (st-(if assignment.baseType is "test" and assignment.start is st then JSON.parse(localStorage["earlyTest"]) else 0))*1000*3600*24 <= midDate.getTime() <= s.end.getTime()+(if listDateOffset is 0 then timeafter else 24*3600*1000)
            e.classList.add "listDisp"
