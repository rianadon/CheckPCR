    ...
    display = (doScroll=true) ->
      ...
      for s in split
        ...
        if s.assignment.end is "Forever"
          ...
          if Math.floor(s.start/1000/3600/24)-(if assignment.baseType is "test"  and start is s.start then JSON.parse(localStorage["earlyTest"]) else 0) <= today <= Math.floor(s.end/1000/3600/24)
            e.classList.add "listDisp"
