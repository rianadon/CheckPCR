...
This function will convert the array of assignments generated by *parse* into readable HTML.
    ...
    display = ->
      ...
      for s in split
        ...
        e.setAttribute "data-class", if isNaN(assignment.class) then "Todo" else window.data.classes[assignment.class]
...
Additionally, the active class needs to be added when inputs are selected (for the login box).
    ...
    for input in document.querySelectorAll "input[type=text]:not(#newText), input[type=password], input[type=email], input[type=url], input[type=tel], input[type=number], input[type=search]"
