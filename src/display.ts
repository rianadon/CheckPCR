...
export function display(doScroll: boolean = true): void {
    ...
    split.forEach((s) => {
        ...
        if (!s.custom || !localStorageRead('sepTasks')) {
            ...
            while (true) {
                let found = true
                // tslint:disable-next-line no-loops
                for (const d = new Date(s.start); d <= s.end; d.setDate(d.getDate() + 1)) {
        ...
        if (already != null) { // Assignment already exists
            ...
            if (s.custom && JSON.parse(localStorage.sepTasks)) {
                ...
                if ((s.assignment.start === st) && (s.assignment.end === 'Forever' || s.assignment.end >= today())) {
    ...
    if (weekHeights[todayWkId] != null) {
        ...
        scroll = (h * 30) + 112 + 14
        if (scroll < 50) scroll = 0 // Also show the day headers if today's date is displayed in the first row of the
                                    // calendar
