...
import { classById, getData } from './pcr';
import { addActivity, saveActivity } from './plugins/activity';
import { extraToTask, getExtra } from './plugins/customAssignments';
...
export function getTimeAfter(date) {
...
export function display(doScroll = true) {
    ...
    const lastData = localStorageRead('data');
    let wk = null;
    // tslint:disable-next-line no-loops
        ...
        if (lastData != null) {
            ...
            if (oldAssignment) {
                if (oldAssignment.body !== assignment.body) {
                    addActivity('edit', oldAssignment, new Date(), true, oldAssignment.class != null ? lastData.classes[oldAssignment.class] : undefined);
    ...
    if (lastData != null) {
        ...
        lastData.assignments.forEach((assignment) => {
            addActivity('delete', assignment, new Date(), true, assignment.class != null ? lastData.classes[assignment.class] : undefined);
    ...
    split.forEach((s) => {
        ...
        if (s.assignment.end >= today() && (s.assignment.baseType === 'test' ||
            ...
            const te = element('div', ['upcomingTest', 'assignmentItem', s.assignment.baseType], `<i class='material-icons'>
                                ...
                                <div class='range'>${dateString(end, true)}</div>`, `test${s.assignment.id}`);
        ...
        if (already != null) {
            ...
            already.setAttribute('data-class', s.custom && separateTaskClass ? 'Task' : classById(s.assignment.class));
    ...
    if (weekHeights[todayWkId] != null) {
        ...
        scroll = (h * 30) + 112 + 14;
        if (scroll < 50)
            scroll = 0; // Also show the day headers if today's date is displayed in the first row of the
        // calendar
...
export function formatUpdate(date) {
    const now = new Date();
