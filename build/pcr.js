...
const usernameEl = elemById('username');
const passwordEl = elemById('password');
const rememberCheck = elemById('remember');
 ...
 */
export async function fetch(override = false, data) {
 ...
 */
export async function dologin(val, submitEvt = false) {
...
function parseAssignment(ca) {
    ...
    const b = _$(_$(t.parentNode).parentNode);
...
function parse(doc) {
    ...
    const data = {
        classes: [],
        assignments: [],
        monthView: _$(doc.querySelector('.rsHeaderMonth')).parentNode.classList.contains('rsSelected')
...
export function urlForAttachment(search) {
    return ATTACHMENTS_URL + search;
}
export function getAttachmentMimeType(search) {
