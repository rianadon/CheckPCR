...
import { display, getTimeAfter } from '../display';
import { getListDateOffset } from '../navigation';
import { getAttachmentMimeType, urlForAttachment } from '../pcr';
...
export function computeWeekId(split) {
...
export function createAssignment(split, data) {
    ...
    const e = element('div', ['assignment', assignment.baseType, 'anim'], `<${smallTag}${link ? ` href='${link}' class='linked' target='_blank'` : ''}>
    ...
    const times = element('div', 'range', assignment.start === assignment.end ? dateString(start) : `${dateString(start)} &ndash; ${dateString(end)}`);
    e.appendChild(times);
    if (assignment.attachments.length > 0) {
        const attachments = element('div', 'attachments');
        assignment.attachments.forEach((attachment) => {
            const a = element('a', [], attachment[0]);
    ...
    const body = element('div', 'body', assignment.body.replace(/font-family:[^;]*?(?:Times New Roman|serif)[^;]*/g, ''));
    ...
    recentActivity().forEach((a) => {
        if ((a[0] === 'edit') && (a[1].id === assignment.id)) {
            const te = element('div', ['innerActivity', 'assignmentItem', assignment.baseType], `<i class='material-icons'>edit</i>
...
export function closeOpened(evt) {
    evt.stopPropagation();
