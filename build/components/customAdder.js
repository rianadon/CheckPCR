...
const newText = elemById('newText');
export function updateNewTips(val, scroll = true) {
    if (scroll) {
                    ...
                    getClasses().forEach((cls) => {
                        ...
                        if (spaceIndex === (val.length - 1)) {
                            ...
                            if (e) {
                                ...
                                const container = element('div', ['classTip', 'active', 'tip'], `<i class='material-icons'>class</i><span class='typed'>${cls}</span>`, id);
                                container.addEventListener('click', tipComplete(cls));
                                elemById('newTips').appendChild(container);
                            }
                        }
                        else {
                            elemById(id).classList.toggle('active', cls.toLowerCase().includes(val.toLowerCase().substr(spaceIndex + 1)));
                        }
                    });
                }
            }
