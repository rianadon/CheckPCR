...
import { decrementListDateOffset, getListDateOffset, incrementListDateOffset, setListDateOffset, zeroListDateOffset } from './navigation';
...
import { _$, _$h, dateString, elemById, element, forceLayout, localStorageRead, localStorageWrite, requestIdleCallback, ripple } from './util';
...
const assignmentColors = localStorageRead('assignmentColors', {
...
if (localStorageRead('data') != null) {
    localStorageRead('data').classes.forEach((c) => {
...
const colorType = _$(document.querySelector(`input[name=\"colorType\"][value=\"${localStorage.colorType}\"]`));
colorType.checked = true;
Array.from(document.getElementsByName('colorType')).forEach((c) => {
    c.addEventListener('change', (evt) => {
        const v = _$(document.querySelector('input[name="colorType"]:checked')).value;
...
dragTarget.on('panend', (e) => {
    if (e.pointerType === 'touch') {
        ...
        if ((menuOut && (velocityX <= 0.3)) || (velocityX < -0.5)) {
            ...
            setTimeout(() => elemById('sideBackground').style.display = 'none', 350);
        }
    }
});
dragTarget.on('tap', (e) => {
...
Object.entries(activityTypes).forEach(([type, enabled]) => {
    ...
    const selectEl = elemById(type + 'Select');
    selectEl.checked = enabled;
    if (enabled) {
        elemById('infoActivity').classList.add(type);
    }
...
const showDoneTasksEl = elemById('showDoneTasks');
...
const onNewTask = () => {
    updateNewTips(elemById('newText').value = '');
    document.body.style.overflow = 'hidden';
...
elemById('newDialog').addEventListener('submit', (evt) => {
    evt.preventDefault();
    const text = elemById('newText').value;
...
elemById('newText').addEventListener('input', () => {
    return updateNewTips(elemById('newText').value);
});
