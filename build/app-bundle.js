...
function logInfo() {
    ...
    console.log(`%cVersion ${app["a" /* VERSION */]} (Check below for latest version)`, 'font-size: 1.1em');
...
function exportVariables() {
    const wnd = window;
    wnd.fetchData = pcr["c" /* fetch */];
    wnd.state = state["d" /* state */];
    wnd.snackbar = snackbar["a" /* snackbar */];
    wnd.displayError = errorDisplay["b" /* displayError */];
    wnd.closeError = errorDisplay["a" /* closeError */];
    Object.keys(state["d" /* state */]).forEach((item) => {
        Object.defineProperty(window, item, {
            get() { return Object(state["b" /* getStateItem */])(item).get(); },
            set(value) { return Object(state["b" /* getStateItem */])(item).set(value); }
        });
    });
}
function initConsoleInteractivity() {
    logInfo();
    exportVariables();
}
...
Object(util["g" /* elemById */])('background').addEventListener('click', components_assignment["b" /* closeOpened */]);
            ...
            if (state["d" /* state */].viewTrans.get()) {
                ...
                const step = (timestamp) => {
                    ...
                    assignments.forEach((assignment, n) => {
                        ...
                        if (start == null) {
                            const [eName, sName] = Object(components_assignment["e" /* getES */])(assignment);
...
window.addEventListener('keydown', (evt) => {
    if (evt.which === 27) { // Escape key pressed
        if (document.getElementsByClassName('full').length !== 0) {
            return Object(components_assignment["b" /* closeOpened */])(new Event('Generated Event'));
        }
    }
});
...
Object(util["g" /* elemById */])('errorNo').addEventListener('click', errorDisplay["a" /* closeError */]);
Object(util["g" /* elemById */])('errorBackground').addEventListener('click', errorDisplay["a" /* closeError */]);
// <a name="new"/>
...
Object(util["g" /* elemById */])('newDialog').addEventListener('submit', (evt) => {
    ...
    const { text, cls, due, st } = Object(customAssignments["c" /* parseCustomTask */])(itext);
    let end = 'Forever';
    ...
    state["d" /* state */].extra.forceUpdate();
    closeNew();
